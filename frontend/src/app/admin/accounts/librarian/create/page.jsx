...
const CreateLibrarianPage = () => {
  ...
  const libraryOptions = [
    { value: 'Thư viện Trung tâm', label: 'Thư viện Trung tâm' },
    { value: 'Thư viện Khoa học', label: 'Thư viện Khoa học' },
    { value: 'Thư viện Kỹ thuật', label: 'Thư viện Kỹ thuật' },
    { value: 'Thư viện Y học', label: 'Thư viện Y học' },
    { value: 'Thư viện Luật', label: 'Thư viện Luật' },
  ];
  
  // Position options
  const positionOptions = [
    { value: 'Thủ thư trưởng', label: 'Thủ thư trưởng' },
    { value: 'Thủ thư phó', label: 'Thủ thư phó' },
  ...
  const validateForm = () => {
    ...
    if (!formData.department) {
      newErrors.department = 'Thư viện là bắt buộc';
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // In a real app, this would call an API to create the librarian account
      console.log('Creating librarian account:', formData);
      ...
      setNotification({
        show: true,
        message: 'Tạo nhân viên thư viện thành công',
        type: 'success'
      });
      
      // Redirect to librarian list after a short delay
      setTimeout(() => {
        router.push('/admin/accounts/librarian');
      }, 1500);
    } catch (error) {
      setNotification({
        show: true,
        message: 'Có lỗi xảy ra khi tạo nhân viên thư viện: ' + error.message,
        type: 'error'
      });
    } finally {
      ...
      <div className="flex items-center space-x-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-sage-900 dark:text-sage-100">
            Thêm nhân viên thư viện mới
          </h1>
          <p className="text-sage-600 dark:text-sage-400 mt-1">
            Điền thông tin nhân viên thư viện để thêm vào hệ thống
          </p>
        </div>
      </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              <label htmlFor="department" className="block text-sm font-medium text-sage-700 dark:text-sage-300 mb-1">
                Thư viện <span className="text-red-500">*</span>
              </label>
              <select
                id="department"
                name="department"
