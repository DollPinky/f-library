...
const LibrarianTab = () => {
  ...
  // Mock data for librarian accounts
  const [librarianAccounts, setLibrarianAccounts] = useState([
    {
      id: 1,
      fullName: 'Phạm Văn D',
      email: 'phamvand@library.university.edu.vn',
      phone: '0123456799',
      employeeCode: 'TV001',
    ...
    },
    {
      id: 3,
      fullName: 'Trần Văn F',
      email: 'tranvanf@library.university.edu.vn',
      phone: '0345678922',
      employeeCode: 'TV003',
      department: 'Thư viện Kỹ thuật',
      position: 'Thủ thư',
      isActive: false,
  ...
  const handleDeleteAccount = (account) => {
    if (window.confirm(`Bạn có chắc chắn muốn xóa tài khoản ${account.fullName}?`)) {
      ...
      setLibrarianAccounts(librarianAccounts.filter(acc => acc.id !== account.id));
  ...
  const handleCreateAccount = () => {
    router.push('/admin/accounts/librarian/create');
  };
    ...
    <div className="space-y-6">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
          <div className="flex-1 max-w-md">
            <div className="relative">
              ...
              <input
                type="text"
                placeholder="Tìm kiếm nhân viên thư viện..."
        ...
        {showFilters && (
          <div className="mt-4 pt-4 border-t border-sage-200 dark:border-sage-800">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div>
                <label className="block text-sm font-medium text-sage-700 dark:text-sage-300 mb-1">Thư viện</label>
                <select
                  className="block w-full rounded-lg border border-sage-300 dark:border-sage-700 bg-white dark:bg-neutral-800 text-sage-900 dark:text-sage-100 shadow-sm focus:ring-sage-500 focus:border-sage-500"
                ...
                >
                  <option value="">Tất cả</option>
                  <option value="Thư viện Trung tâm">Thư viện Trung tâm</option>
                  <option value="Thư viện Khoa học">Thư viện Khoa học</option>
                  <option value="Thư viện Kỹ thuật">Thư viện Kỹ thuật</option>
                </select>
              </div>
      ...
      <div className="bg-white dark:bg-neutral-900 rounded-xl shadow-sm border border-sage-200 dark:border-sage-800 overflow-hidden">
        {loading ? (
          ...
          <TableView
            data={librarianAccounts}
            columns={columns}
            pagination={{
              page: filters.page,
              size: filters.size,
              totalElements: librarianAccounts.length,
              totalPages: Math.ceil(librarianAccounts.length / filters.size)
            }}
      ...
      <DetailDrawer
        ...
        title="Thông tin chi tiết nhân viên thư viện"
      >
        {selectedAccount && (
          <div className="space-y-6">
